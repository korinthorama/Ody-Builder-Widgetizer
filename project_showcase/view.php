...
<style>
    ...
    .wdg-psc-item { border: 1px solid #ccc; border-radius: 4px; margin-bottom: 6px; background: #f0f4f4; }
    .wdg-psc-item-header { display: flex; align-items: center; gap: 8px; padding: 5px 8px; background: #002e3a; border-radius: 4px 4px 0 0; cursor: move; }
    .wdg-psc-item-header span { color: white; font-size: 11px; flex: 1; }
...
jQuery(function ($) {
    ...
    function addItem(itemData) {
        ...
        $body.append(
            ...
            '<input type="text" id="wdg_psc_title_' + uid + '" class="listbox" value="' + $('<div>').text(itemData.title || '').html() + '"></div>'
        );
        ...
        if ($.fn.sortable) {
            $('#wdg_psc_items_list').sortable({
                handle: '.wdg-psc-item-header',
    ...
    function renumberItems() {
        $('#wdg_psc_items_list .wdg-psc-item').each(function(idx) {
            $(this).find('.wdg-psc-item-header span').text('Project ' + (idx + 1));
        });
    }
